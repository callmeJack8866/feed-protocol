...
library Math {
     ...
     *
     * The result is stored in two 256 variables such that sum = high * 2虏鈦碘伓 + low.
     */
     ...
     *
     * The result is stored in two 256 variables such that product = high * 2虏鈦碘伓 + low.
     */
    function mul512(uint256 a, uint256 b) internal pure returns (uint256 high, uint256 low) {
        // 512-bit multiply [high low] = x * y. Compute the product mod 2虏鈦碘伓 and mod 2虏鈦碘伓 - 1, then use
        // the Chinese Remainder Theorem to reconstruct the 512 bit result. The result is stored in two 256
        // variables such that product = high * 2虏鈦碘伓 + low.
        assembly ("memory-safe") {
    ...
    /**
     * @dev Unsigned saturating addition, bounds to `2虏鈦碘伓 - 1` instead of overflowing.
     */
    ...
    /**
     * @dev Unsigned saturating multiplication, bounds to `2虏鈦碘伓 - 1` instead of overflowing.
     */
    ...
    function mulDiv(uint256 x, uint256 y, uint256 denominator) internal pure returns (uint256 result) {
        unchecked {
            ...
            // Make sure the result is less than 2虏鈦碘伓. Also prevents denominator == 0.
            if (denominator <= high) {
            ...
            assembly ("memory-safe") {
                ...
                // Flip twos such that it is 2虏鈦碘伓 / twos. If twos is zero, then it becomes one.
            ...
            // Invert denominator mod 2虏鈦碘伓. Now that denominator is an odd number, it has an inverse modulo 2虏鈦碘伓 such
            // that denominator * inv 鈮?1 mod 2虏鈦碘伓. Compute the inverse by starting with a seed that is correct for
            // four bits. That is, denominator * inv 鈮?1 mod 2鈦?
            uint256 inverse = (3 * denominator) ^ 2;

            // Use the Newton-Raphson iteration to improve the precision. Thanks to Hensel's lifting lemma, this also
            // works in modular arithmetic, doubling the correct bits in each step.
            inverse *= 2 - denominator * inverse; // inverse mod 2鈦?            inverse *= 2 - denominator * inverse; // inverse mod 2鹿鈦?            inverse *= 2 - denominator * inverse; // inverse mod 2鲁虏
            inverse *= 2 - denominator * inverse; // inverse mod 2鈦垛伌
            inverse *= 2 - denominator * inverse; // inverse mod 2鹿虏鈦?            inverse *= 2 - denominator * inverse; // inverse mod 2虏鈦碘伓

            // Because the division is now exact we can divide by multiplying with the modular inverse of denominator.
            // This will give us the correct result modulo 2虏鈦碘伓. Since the preconditions guarantee that the outcome is
            // less than 2虏鈦碘伓, this is the final result. We don't need to compute the high bits of the result and high
    ...
    function invMod(uint256 a, uint256 n) internal pure returns (uint256) {
        unchecked {
            ...
            // ax + ny = 1
            // ax = 1 + (-y)n
            // ax 鈮?1 (mod n) # x is the inverse of a modulo n
     ...
     * prime, then `a**(p-1) 鈮?1 mod p`. As a consequence, we have `a * a**(p-2) 鈮?1 mod p`, which means that
    ...
    function sqrt(uint256 a) internal pure returns (uint256) {
        unchecked {
            ...
            // In this function, we use Newton's method to get a root of `f(x) := x虏 - a`. It involves building a
            // sequence x_n that converges toward sqrt(a). For each iteration x_n, we also define the error between
            // the current value as `蔚_n = | x_n - sqrt(a) |`.
            //
            // For our first estimation, we consider `e` the smallest power of 2 which is bigger than the square root
            // of the target. (i.e. `2**(e-1) 鈮?sqrt(a) < 2**e`). We know that `e 鈮?128` because `(2鹿虏鈦?虏 = 2虏鈦碘伓` is
            // bigger than any uint256.
            //
            // By noticing that
            // `2**(e-1) 鈮?sqrt(a) < 2**e 鈫?(2**(e-1))虏 鈮?a < (2**e)虏 鈫?2**(2*e-2) 鈮?a < 2**(2*e)`
            ...
            // We now have x_n such that `x_n = 2**(e-1) 鈮?sqrt(a) < 2**e = 2 * x_n`. This implies 蔚_n 鈮?2**(e-1).
            //
            // We can refine our estimation by noticing that the middle of that interval minimizes the error.
            // If we move x_n to equal 2**(e-1) + 2**(e-2), then we reduce the error to 蔚_n 鈮?2**(e-2).
            // This is going to be our x_0 (and 蔚_0)
            xn = (3 * xn) >> 1; // 蔚_0 := | x_0 - sqrt(a) | 鈮?2**(e-2)

            // From here, Newton's method give us:
            // x_{n+1} = (x_n + a / x_n) / 2
            //
            // One should note that:
            // x_{n+1}虏 - a = ((x_n + a / x_n) / 2)虏 - a
            //              = ((x_n虏 + a) / (2 * x_n))虏 - a
            //              = (x_n鈦?+ 2 * a * x_n虏 + a虏) / (4 * x_n虏) - a
            //              = (x_n鈦?+ 2 * a * x_n虏 + a虏 - 4 * a * x_n虏) / (4 * x_n虏)
            //              = (x_n鈦?- 2 * a * x_n虏 + a虏) / (4 * x_n虏)
            //              = (x_n虏 - a)虏 / (2 * x_n)虏
            //              = ((x_n虏 - a) / (2 * x_n))虏
            //              鈮?0
            // Which proves that for all n 鈮?1, sqrt(a) 鈮?x_n
            //
            // This gives us the proof of quadratic convergence of the sequence:
            // 蔚_{n+1} = | x_{n+1} - sqrt(a) |
            //         = | (x_n + a / x_n) / 2 - sqrt(a) |
            //         = | (x_n虏 + a - 2*x_n*sqrt(a)) / (2 * x_n) |
            //         = | (x_n - sqrt(a))虏 / (2 * x_n) |
            //         = | 蔚_n虏 / (2 * x_n) |
            //         = 蔚_n虏 / | (2 * x_n) |
            //
            ...
            // 蔚_1 = 蔚_0虏 / | (2 * x_0) |
            //     鈮?(2**(e-2))虏 / (2 * (2**(e-1) + 2**(e-2)))
            //     鈮?2**(2*e-4) / (3 * 2**(e-1))
            //     鈮?2**(e-3) / 3
            //     鈮?2**(e-3-log2(3))
            //     鈮?2**(e-4.5)
            //
            // For the following iterations, we use the fact that, 2**(e-1) 鈮?sqrt(a) 鈮?x_n:
            // 蔚_{n+1} = 蔚_n虏 / | (2 * x_n) |
            //         鈮?(2**(e-k))虏 / (2 * 2**(e-1))
            //         鈮?2**(2*e-2*k) / 2**e
            //         鈮?2**(e-2*k)
            xn = (xn + a / xn) >> 1; // 蔚_1 := | x_1 - sqrt(a) | 鈮?2**(e-4.5)  -- special case, see above
            xn = (xn + a / xn) >> 1; // 蔚_2 := | x_2 - sqrt(a) | 鈮?2**(e-9)    -- general case with k = 4.5
            xn = (xn + a / xn) >> 1; // 蔚_3 := | x_3 - sqrt(a) | 鈮?2**(e-18)   -- general case with k = 9
            xn = (xn + a / xn) >> 1; // 蔚_4 := | x_4 - sqrt(a) | 鈮?2**(e-36)   -- general case with k = 18
            xn = (xn + a / xn) >> 1; // 蔚_5 := | x_5 - sqrt(a) | 鈮?2**(e-72)   -- general case with k = 36
            xn = (xn + a / xn) >> 1; // 蔚_6 := | x_6 - sqrt(a) | 鈮?2**(e-144)  -- general case with k = 72

            // Because e 鈮?128 (as discussed during the first estimation phase), we know have reached a precision
            // 蔚_6 鈮?2**(e-144) < 1. Given we're operating on integers, then we can ensure that xn is now either
            // sqrt(a) or sqrt(a) + 1.
...
// File @openzeppelin/contracts-upgradeable/token/ERC721/ERC721Upgradeable.sol@v5.4.0
...
// File @openzeppelin/contracts-upgradeable/token/ERC721/extensions/ERC721EnumerableUpgradeable.sol@v5.4.0
...
// File @openzeppelin/contracts-upgradeable/token/ERC721/extensions/ERC721URIStorageUpgradeable.sol@v5.4.0
...
// File contracts/FeedConsensus.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.28;
/**
 * @title FeedConsensus
 * @dev 鍏辫瘑寮曟搸 鈥?Commit-Reveal 涓ら樁娈典环鏍兼彁浜?鈥?UUPS 鍙崌绾?
 *
 * 娴佺▼锛?
 * 1. Commit 闃舵锛氬杺浠峰憳鎻愪氦 priceHash = keccak256(price, salt)
 * 2. Reveal 闃舵锛氬杺浠峰憳鎻ず price + salt锛屽悎绾﹂獙璇佸搱甯?
 * 3. 鍏辫瘑璁＄畻锛氬幓鏋佸€间腑浣嶆暟锛堥摼涓嬭绠楋紝閾句笂楠岃瘉绛惧悕锛?
 * 4. 缁撶畻锛氭牴鎹叡璇嗕环鏍肩粨绠楄鍗?
 */
    ...
    ReentrancyGuardUpgradeable
{
    ...
    struct PriceCommit {
        ...
        uint256 revealedPrice; // 鎻ず鍚庣殑浠锋牸 (8 decimals)
        bool committed;
        bool revealed;
        uint256 commitTime;
    }
    ...
    /**
     * @notice 鍒濆鍖?
     * @param owner_ 鍚堢害鎵€鏈夎€?
     * @param commitWindow_ Commit 闃舵鏃堕暱锛堢锛?
     * @param revealWindow_ Reveal 闃舵鏃堕暱锛堢锛?
     */
    function initialize(
        address owner_,
    ...
    /**
     * @notice 鍒涘缓鍠備环璁㈠崟
     * @param orderId 璁㈠崟鍞竴鏍囪瘑
     * @param symbol 鏍囩殑浠ｇ爜
     * @param notionalAmount 鍚嶄箟鏈噾
     * @param quorum 娉曞畾浜烘暟
     */
    ...
    /**
     * @notice 鎵归噺鎻愪氦浠锋牸鍝堝笇
     * @param orderIds 璁㈠崟 ID 鏁扮粍
     * @param priceHashes 瀵瑰簲鐨勪环鏍煎搱甯屾暟缁?
     */
    ...
    /**
     * @notice 鎻ず浠锋牸锛圧eveal 闃舵锛?
     * @param orderId 璁㈠崟 ID
     * @param price 瀹為檯浠锋牸 (8 decimals)
     * @param salt 闅忔満鐩愬€?
     */
    ...
    /**
     * @notice 鎻愪氦鍏辫瘑浠锋牸锛堢敱 operator 鍦ㄩ摼涓嬭绠楀悗鎻愪氦锛?
     * @param orderId 璁㈠崟 ID
     * @param consensusPrice 鍏辫瘑浠锋牸 (8 decimals)
     */
    ...
    /**
     * @notice 璁＄畻浠锋牸鍝堝笇锛堣緟鍔╁嚱鏁帮紝鍓嶇鍙敤锛?
     * @param price 浠锋牸 (8 decimals)
     * @param salt 闅忔満鐩愬€?
     */
...
// File contracts/FeederLicense.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.28;
/**
 * @title FeederLicense
 * @dev BEP-721 NFT 鎵х収 鈥?UUPS 鍙崌绾?
 *
 * 鍔熻兘锛?
 * - 姣忎釜鍠備环鍛橀渶鎸佹湁涓€涓?NFT 鎵х収鎵嶈兘鍙備笌鍠備环
 * - 浠?authorized minters (FeedEngine 涓诲悎绾? 鍙摳閫?
 * - tokenURI 鈫?IPFS 鍏冩暟鎹?
 * - 鏀寔鎸?owner 鏌ヨ鍏ㄩ儴 token
 */
    ...
    UUPSUpgradeable
{
    ...
    /**
     * @notice 璁剧疆/鍙栨秷鎺堟潈閾搁€犺€?
     * @param minter 閾搁€犺€呭湴鍧€
     * @param authorized 鏄惁鎺堟潈
     */
     ...
     * @notice 閾搁€犳墽鐓?NFT
     * @param to 鎺ユ敹鑰呭湴鍧€
     * @param uri IPFS 鍏冩暟鎹?URI
     * @param licenseType_ 鎵х収绫诲瀷
     * @return tokenId 鏂伴摳閫犵殑 tokenId
     */
    function mint(
        address to,
        string calldata uri,
    ...
    /**
     * @notice 閿€姣佹墽鐓э紙浠呮墍鏈夎€呮垨鎺堟潈閾搁€犺€咃級
     * @param tokenId 瑕侀攢姣佺殑 tokenId
     */
    ...
    /**
     * @notice 鑾峰彇鐢ㄦ埛鎸佹湁鐨勬墍鏈?tokenId
     * @param owner_ 鐢ㄦ埛鍦板潃
     * @return tokenIds tokenId 鏁扮粍
     */
...
// File contracts/RewardPenalty.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.28;
/**
 * @title RewardPenalty
 * @dev 濂栨儵绯荤粺 鈥?UUPS 鍙崌绾?
 *
 * 濂栧姳鍒嗛厤姣斾緥锛堟柟妗堣姹傦級锛?
 * - 70% 鍠備环鍛?
 * - 10% 骞冲彴閲戝簱
 ...
 *
 * 鎯╃綒绛夌骇锛?
 * - WARNING  (0)锛氬彛澶磋鍛婏紝鏃犲疄闄呮墸鍑?
 * - MINOR    (1)锛氭墸闄よ川鎶?5%
 * - MAJOR    (2)锛氭墸闄よ川鎶?20% + 绂佹鎶㈠崟 7 澶?
 * - CRITICAL (3)锛氭墸闄よ川鎶?50% + 姘镐箙灏佺
 */
    ...
    ReentrancyGuardUpgradeable
{
    ...
    struct PenaltyRecord {
        ...
        uint256 timestamp;
        uint256 banUntil;     // 绂佹鎶㈠崟鎴鏃堕棿 (0=鏃犻檺鏈?
    }

    /// @notice 濂栧姳鍒嗛厤姣斾緥 (basis points)
    struct RewardSplit {
        uint256 feederBps;    // 鍠備环鍛樻瘮渚?
        uint256 platformBps;  // 骞冲彴姣斾緥
        uint256 daoBps;       // DAO 姣斾緥
        uint256 burnBps;      // 閿€姣佹瘮渚?
    }
     ...
     * @notice 鍒濆鍖?
     * @param owner_ 鎵€鏈夎€?
     * @param feedToken_ FEED 浠ｅ竵鍦板潃
     * @param platformTreasury_ 骞冲彴閲戝簱
     * @param daoTreasury_ DAO 閲戝簱
     */
    function initialize(
        address owner_,
        address feedToken_,
        ...
        penaltyBanDays[PenaltyLevel.CRITICAL] = 0; // 姘镐箙
    }
    ...
    /**
     * @notice 鍒嗛厤璁㈠崟濂栧姳缁欏涓杺浠峰憳
     * @param orderId 璁㈠崟ID
     * @param feeders 鍠備环鍛樺湴鍧€鏁扮粍
     * @param totalReward 鎬诲鍔遍噾棰?
     */
    function distributeRewards(
        ...
        // 閿€姣侀儴鍒嗭細杞埌 address(0xdead) 妯℃嫙閿€姣?
        if (burnAmount > 0) {
    ...
    /**
     * @notice 瀵瑰杺浠峰憳鏂藉姞鎯╃綒
     * @param feeder 鍠備环鍛樺湴鍧€
     * @param level 鎯╃綒绛夌骇
     * @param reason 鎯╃綒鍘熷洜
     * @param stakedAmount 璇ュ杺浠峰憳褰撳墠璐ㄦ娂閲戦锛堢敤浜庤绠楁墸闄わ級
     * @return slashedAmount 瀹為檯鎵ｉ櫎閲戦
     */
...
// File contracts/FeedEngine.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.28;
/**
 * @title FeedEngine
 * @dev 涓诲悎绾︼紙闂ㄩ潰妯″紡锛?鈥?UUPS 鍙崌绾?
 *
 * 鏁村悎瀛愬悎绾﹀姛鑳斤細
 * - 鍠備环鍛樻敞鍐?+ 绛夌骇绠＄悊
 * - 璁㈠崟鍒涘缓 + 鎶㈠崟 + 缁撶畻
 * - 璐ㄦ娂 (stake/unstake/withdraw)
 * - 璋冪敤 FeedConsensus 杩涜鍏辫瘑
 * - 璋冪敤 RewardPenalty 杩涜濂栨儵
 */
    ...
    ReentrancyGuardUpgradeable
{
    ...
    struct FeederInfo {
        ...
        uint256 totalFeeds;
        uint256 stakedAmount;
        uint256 unstakeRequestTime; // 瑙ｉ攣璇锋眰鏃堕棿
        uint256 dailyGrabCount;     // 褰撴棩鎶㈠崟鏁?
        uint256 lastGrabDate;       // 涓婃鎶㈠崟鏃ユ湡
    }
    ...
    function initialize(
        ...
        maxDailyGrabByRank[FeederRank.S] = type(uint256).max; // 鏃犻檺
    ...
    /**
     * @notice 娉ㄥ唽涓哄杺浠峰憳
     * @dev 闇€鍏?approve FEED 浠ｅ竵鐢ㄤ簬璐ㄦ娂
     * @param stakeAmount 鍒濆璐ㄦ娂閲戦
     */
    ...
    /**
     * @notice 缁撶畻璁㈠崟锛堢敱 operator/owner 璋冪敤锛?
     * @param orderId 璁㈠崟 ID
     * @param rewardAmount 濂栧姳鎬婚
     */
