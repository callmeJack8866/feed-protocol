...
const MOCK_CASES: ArbitrationCase[] = [
  {
    ...
    evidenceUrl: 'https://picsum.photos/seed/market/800/400',
    votes: { up: 12, down: 2 }
  }
];
    ...
    <div className="space-y-12">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <h2 className="text-4xl font-black font-orbitron tracking-tighter italic uppercase text-rose-500">JUDICIAL CHAMBER</h2>
          <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">A/S Class Feeders Only · Dispute Resolution Protocol</p>
        </div>
        <div className="flex bg-rose-500/10 border border-rose-500/20 px-6 py-3 rounded-2xl items-center gap-4">
           <span className="text-rose-500 animate-pulse">●</span>
           <span className="text-[10px] font-black uppercase tracking-widest text-rose-400">1 Active Conflict Detected</span>
        </div>
      </header>
          ...
          >
            <div className="flex justify-between items-start">
               ...
               <div className="text-right">
                  <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">Staked Bounty</p>
            ...
            <div className="p-5 rounded-2xl bg-black/40 border border-rose-500/10 space-y-2">
               <p className="text-[10px] text-rose-500 font-black uppercase tracking-widest">Conflict Root</p>
            ...
            <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-slate-500">
               ...
               <span className="text-rose-500/60 font-orbitron">01:42:55 UNTIL LOCK</span>
            </div>
          </motion.div>
        ))}
      </div>
             ...
             >
                <div className="p-10 border-b border-rose-500/10 flex justify-between items-center bg-rose-500/[0.03]">
                   <div className="space-y-1">
                      <h2 className="text-3xl font-black font-orbitron text-rose-500 italic uppercase">ARBITRATION PROTOCOL</h2>
                      <p className="text-[10px] text-slate-500 font-black tracking-widest uppercase">Verified Evidence vs Submitted Signatures</p>
                   </div>
                ...
                <div className="flex-1 overflow-y-auto p-12 custom-scrollbar grid grid-cols-1 lg:grid-cols-2 gap-12">
                   ...
                   <div className="space-y-8">
                      <section className="space-y-4">
                        <h3 className="text-xs font-black text-rose-500 uppercase tracking-widest px-2">Primary Evidence Source</h3>
                      ...
                      <section className="space-y-4">
                        <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest px-2">Dispute Summary</h3>
                   ...
                   <div className="space-y-8">
                      <section className="space-y-4">
                        <h3 className="text-xs font-black text-cyan-500 uppercase tracking-widest px-2">Aggregated Submissions</h3>
                        <div className="space-y-3">
                           {selectedCase.submittedPrices.map((sub, i) => (
                             <div key={i} className="p-5 rounded-2xl border border-white/5 bg-white/5 flex justify-between items-center group hover:border-cyan-500/30 transition-all">
                                <div>
                                   <p className="text-[9px] text-slate-500 font-mono uppercase">Feeder ID</p>
                                   <p className="text-sm font-bold text-slate-300 font-mono">{sub.feeder}</p>
                                </div>
                                <div className="text-right">
                                   <p className="text-[9px] text-slate-500 font-black uppercase mb-0.5">Reported Price</p>
                           ...
                           <div className="p-6 rounded-2xl border-2 border-dashed border-white/5 text-center flex flex-col items-center justify-center space-y-2 opacity-50">
                              <span className="text-xs font-black uppercase tracking-widest">8 Additional Nodes Halted</span>
                              <span className="text-[8px] font-mono uppercase">Waiting for consensus freeze...</span>
                           </div>
                        </div>
                      </section>
                ...
                <div className="p-8 border-t border-rose-500/10 flex items-center justify-between bg-black/40">
                   <div className="flex items-center gap-6">
                      <div className="flex -space-x-3">
                         {[1,2,3,4].map(i => (
                           <div key={i} className="w-10 h-10 rounded-full border-2 border-slate-900 overflow-hidden">
                              <img src={`https://picsum.photos/seed/${i*50}/100/100`} alt="judges" />
                           </div>
                         ))}
                      ...
                      <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">12 Active Arbitrators Connected</p>
                   </div>
                   <p className="text-[10px] text-rose-500/60 font-black uppercase tracking-widest">Criticality: S-Tier Enforcement Required</p>
                </div>
             </motion.div>
          </div>
        )}
