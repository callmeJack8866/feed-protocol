...
type ParticipantStatus = 'COMMITTED' | 'VERIFYING' | 'PENDING' | 'SYNCING';
...
const OrderDetailModal: React.FC<OrderDetailModalProps> = ({ order, onClose, onGrab }) => {
  ...
  const range = useMemo(() => {
    ...
    let tolerance = 0.20; 
    if (order.market === MarketType.CRYPTO) tolerance = 0.40; 
    if (order.market === MarketType.FOREX) tolerance = 0.05; 
    if (isMasterZone) tolerance = tolerance * 0.25; 
  ...
  useEffect(() => {
    ...
    const updateLogs = (msg: string) => {
      setLogLines(prev => [...prev.slice(-8), msg]);
    };
    ...
    if (isCrypto) {
      ...
      wsRef.current.onmessage = (event) => {
        ...
        if (lastPriceRef.current !== null) {
          setPriceDirection(price > lastPriceRef.current ? 'up' : price < lastPriceRef.current ? 'down' : 'stable');
        }
        
        setLivePrice(price);
        setPriceHistory(prev => [...prev.slice(-29), price]);
        lastPriceRef.current = price;
        updateLogs(`[DATA] Tick Received: ${price.toFixed(2)} | Qty: ${parseFloat(data.q).toFixed(4)}`);
        setSignalStrength(prev => Math.max(85, Math.min(99, prev + (Math.random() - 0.5) * 4)));
      };
      ...
      const tickerInterval = setInterval(() => {
        const basePrice = lastPriceRef.current || range.ref;
        const volatility = order.notionalAmount > 1000000 ? 0.0001 : 0.0005;
        const change = basePrice * volatility * (Math.random() - 0.5);
        const newPrice = basePrice + change;
        
        setPriceDirection(newPrice > basePrice ? 'up' : 'down');
        setLivePrice(newPrice);
        setPriceHistory(prev => [...prev.slice(-29), newPrice]);
        lastPriceRef.current = newPrice;
        
        const type = Math.random() > 0.8 ? 'PUSH' : 'SCAN';
        updateLogs(`[${type}] Local Node Proxy: ${newPrice.toFixed(4)}`);
        setSignalStrength(prev => Math.max(80, Math.min(98, prev + (Math.random() - 0.5) * 5)));
  ...
  }, [order.symbol, order.market, order.exchange, range.ref]);

  const getConditionIcon = (type: ConditionType) => {
    switch (type) {
      case ConditionType.CONSECUTIVE_LIMIT: return '🔥';
      case ConditionType.EX_DIVIDEND: return '📉';
      case ConditionType.LIMIT_UP: return '🔴';
      case ConditionType.LIMIT_DOWN: return '🟢';
      case ConditionType.VOLATILITY_HIGH: return '⚡';
      default: return '🛡️';
    }
  };
  ...
  const participants = useMemo(() => {
    ...
    for (let i = 0; i < order.requiredFeeders; i++) {
      const id = `NODE-${1000 + i}`;
      const latency = `${(Math.random() * 45 + 5).toFixed(1)}ms`;
      const reliability = 95 + Math.random() * 5;
      if (i < committedCount) {
        list.push({ id, status: 'COMMITTED', latency, reliability });
      } else if (i === committedCount) {
        list.push({ id, status: 'VERIFYING', latency, reliability });
      } else {
        list.push({ id, status: 'PENDING', latency, reliability });
      }
    }
    return list;
  }, [order.requiredFeeders]);

  const committedCount = participants.filter(p => p.status === 'COMMITTED').length;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/98 backdrop-blur-3xl p-4 lg:p-8">
      <motion.div 
          ...
          <div className="space-y-8">
            <div className="flex items-start gap-10">
              <motion.div 
                ...
                className="w-32 h-32 rounded-[3rem] bg-white flex items-center justify-center text-6xl shadow-2xl shrink-0"
              >
               ...
               <div className="mt-8 flex items-baseline gap-4">
                  <AnimatePresence mode="wait">
                    <motion.span 
                      ...
                      className={`text-5xl font-black font-orbitron tracking-tighter ${priceDirection === 'up' ? 'text-emerald-400' : priceDirection === 'down' ? 'text-rose-400' : 'text-white'}`}
                    >
                   ...
                   <div key={p.id} className="relative group">
                     <motion.div 
                        ...
                        className={`aspect-square rounded-2xl border flex items-center justify-center text-[10px] font-black transition-all duration-500 ${
                          p.status === 'COMMITTED' ? 'bg-cyan-500 border-cyan-400 text-black shadow-[0_0_25px_rgba(34,211,238,0.4)]' :
                          p.status === 'VERIFYING' ? 'bg-amber-500/10 border-amber-500/40 text-amber-500 animate-pulse' :
                          'bg-white/5 border-white/5 text-slate-800'
                        }`}
                     >
                        {p.status === 'COMMITTED' ? '✓' : i + 1}
                     </motion.div>
                   </div>
                 ))}
               </div>
