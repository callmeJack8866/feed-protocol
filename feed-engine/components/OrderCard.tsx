...
import { FeedOrder, OrderStatus, ConditionType, SpecialCondition } from '../types';
...
const CONDITION_STYLE_BASE: Record<ConditionType, { icon: string; labelKey: keyof TranslationKeys['condition']; bgColor: string; textColor: string; borderColor: string }> = {
  [ConditionType.LIMIT_UP]: { icon: '⬆️', labelKey: 'limitUp', bgColor: 'bg-red-500/20', textColor: 'text-red-400', borderColor: 'border-red-500/30' },
  [ConditionType.LIMIT_DOWN]: { icon: '⬇️', labelKey: 'limitDown', bgColor: 'bg-emerald-500/20', textColor: 'text-emerald-400', borderColor: 'border-emerald-500/30' },
  [ConditionType.CONSECUTIVE_LIMIT]: { icon: '🔥', labelKey: 'consecutiveLimit', bgColor: 'bg-red-600/25', textColor: 'text-red-300', borderColor: 'border-red-400/40' },
  [ConditionType.SUSPENSION]: { icon: '⏸️', labelKey: 'suspension', bgColor: 'bg-slate-500/20', textColor: 'text-slate-400', borderColor: 'border-slate-500/30' },
  [ConditionType.PRICE_ADJUSTMENT]: { icon: '⚙️', labelKey: 'priceAdjustment', bgColor: 'bg-amber-500/20', textColor: 'text-amber-400', borderColor: 'border-amber-500/30' },
  [ConditionType.EX_DIVIDEND]: { icon: '📊', labelKey: 'exDividend', bgColor: 'bg-amber-500/20', textColor: 'text-amber-400', borderColor: 'border-amber-500/30' },
  [ConditionType.VOLATILITY_HIGH]: { icon: '⚡', labelKey: 'volatilityHigh', bgColor: 'bg-purple-500/20', textColor: 'text-purple-400', borderColor: 'border-purple-500/30' },
  [ConditionType.SPECIAL_TREATMENT]: { icon: '⚠️', labelKey: 'specialTreatment', bgColor: 'bg-red-500/20', textColor: 'text-red-400', borderColor: 'border-red-500/30' },
};
...
function getZoneBadge(notionalAmount: number): { label: string; icon: string; color: string } {
  if (notionalAmount >= 1000000) return { label: 'MASTER', icon: '💎', color: 'bg-gradient-to-r from-amber-500 to-yellow-400 text-black' };
  if (notionalAmount >= 100000) return { label: 'COMPETITIVE', icon: '🔥', color: 'bg-gradient-to-r from-orange-500 to-red-500 text-white' };
  return { label: 'BEGINNER', icon: '🌀', color: 'bg-gradient-to-r from-cyan-500 to-blue-500 text-black' };
}
        ...
        {conditions.length > 0 && (
          <div className="absolute top-10 left-10 flex flex-col gap-2 z-20">
            {conditions.map((cond, i) => (
              <ConditionBadge key={i} condition={cond} conditionLabels={t.condition} />
            ))}
          </div>
        )}

        {/* Floating System Badges — 右上角 */}
      ...
      <div className="flex-1 p-12 pt-20 flex flex-col justify-between relative bg-gradient-to-b from-transparent to-cyan-900/5">
        <div className="space-y-4">
          ...
          <p className="text-xs text-slate-400 font-medium leading-relaxed italic opacity-70 group-hover:opacity-100 transition-opacity line-clamp-2">
            {conditions.length > 0
              ? conditions.map(c => { const s = CONDITION_STYLE_BASE[c.type]; return s ? t.condition[s.labelKey] : c.description; }).join(' · ') + ` — ${order.symbol}`
