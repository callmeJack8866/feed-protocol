      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="px-6 py-3 bg-white/5 border border-white/10 rounded-2xl flex items-center gap-4">
           <div className="text-right">
              <p className="text-[10px] text-slate-500 font-black uppercase">{t.staking.seasonEnd}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
           ...
           <div className="p-10 rounded-[3rem] glass-panel border border-emerald-500/20 relative overflow-hidden group">
              ...
              <div className="relative z-10 flex flex-col md:flex-row justify-between gap-10">
                 <div className="space-y-8">
                    <div>
                       <p className="text-xs text-slate-500 uppercase font-black tracking-widest mb-1">{t.staking.activeGuarantee}</p>
                    ...
                    <div className="flex gap-10">
                       <div className="space-y-1">
                          <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest">{t.staking.protocolTrust}</p>
                          <p className="text-xl font-black font-orbitron text-white">98.4%</p>
                       </div>
                       <div className="space-y-1">
                          <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest">{t.staking.marketStatus}</p>
                          <p className="text-xl font-black font-orbitron text-emerald-400">{t.staking.active}</p>
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="w-full md:w-64 p-8 bg-black/40 rounded-[2.5rem] border border-white/5 space-y-6 flex flex-col justify-center">
                    <div className="space-y-1 text-center">
                       <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">{t.staking.withdrawalCooldown}</p>
                       <p className="text-3xl font-black font-orbitron text-white">30 <span className="text-sm opacity-50 uppercase tracking-tighter italic">{t.staking.days}</span></p>
                    </div>
           ...
           <div className="p-8 rounded-[3rem] glass-panel border border-rose-500/20 bg-rose-500/[0.02]">
              <div className="flex justify-between items-center mb-6">
                 <h3 className="text-xs font-black text-rose-500 uppercase tracking-widest px-2 flex items-center gap-2">
                    ...
                    {t.staking.slashingRisk}
                 </h3>
                 <span className="text-[10px] text-slate-500 font-black uppercase">{t.staking.lowExposure}</span>
              </div>
              ...
              <p className="text-[10px] text-slate-500 leading-relaxed font-medium text-center">
                 {t.staking.riskDescription}
                 <span className="text-emerald-500 ml-1">{t.staking.safeZone}</span>
              </p>
           </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { tier: t.staking.basicRequirement, amount: 100, access: t.staking.beginnerZoneAccess, color: 'slate' },
                { tier: t.staking.proRequirement, amount: 1000, access: t.staking.competitiveZoneAccess, color: 'cyan' },
                { tier: t.staking.eliteRequirement, amount: 10000, access: t.staking.arbitrationRights, color: 'rose' }
              ].map(tier => (
                <div key={tier.tier} className={`p-8 rounded-[2.5rem] glass-panel border group hover:scale-[1.02] transition-transform ${
                  ...
                  tier.color === 'cyan' ? 'border-cyan-500/20' : 'border-white/5'
                   ...
                   <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest mb-1">{tier.tier}</p>
        ...
        <div className="p-10 rounded-[3.5rem] glass-panel border border-white/10 space-y-10 flex flex-col justify-between">
           <div className="space-y-8">
              ...
              <div className="space-y-4">
                <label className="text-[10px] text-slate-500 font-black uppercase tracking-widest px-1">{stakeMode === 'stake' ? t.staking.amountToStake : t.staking.amountToWithdraw}</label>
                ...
                <div className="flex justify-between items-center text-[10px] text-slate-500 font-black uppercase tracking-widest px-1">
                   <span>{t.staking.available}: ${profile.balanceUSDT.toLocaleString()}</span>
                   <button className="text-cyan-500 hover:text-cyan-400 transition-colors">{t.staking.max}</button>
                </div>
              </div>
           </div>
           ...
           <div className="space-y-6">
              ...
              <button className={`w-full py-6 rounded-[2.2rem] font-black font-orbitron text-lg transition-all shadow-2xl active:scale-95 italic ${
                ...
                {stakeMode === 'stake' ? t.staking.initializeStake : t.staking.requestRelease}
              </button>
           </div>
        </div>
      </div>
